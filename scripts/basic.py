...
parser.add_option('-t', '--token-text', action="store_true",
                  dest="token_text", default=False)
parser.add_option('-b', '--token-bases', action="store_true",
                  dest="token_bases", default=False)
parser.add_option('-s', '--token-speech', action="store_true",
                  dest="token_speech", default=False)
...
def ccl_poses(ccl):
    tree = ET.fromstring(ccl)
    return [tok.find('./lex/ctag').text.split(":")[0]
